...
已测代码：[`087ba6b`](https://github.com/myp81607-dot/pdf-invoice-reviewer/commit/087ba6b0b3c41f18581bccecde0ddd7c58b6bf2e)，[CI 完整通过](https://github.com/myp81607-dot/pdf-invoice-reviewer/actions/runs/35446565476)，环境为 Ubuntu 24.04.5、Python 3.12.14、Node 22.23.2。本次文档更新没有修改这份已测代码。

| 检查命令 | 结果 |
|---|---|
| `python -m pytest -q` | 22 项通过，2 条警告 |
| `node --test tests/drafts.test.mjs` | 4 项通过 |
| `python scripts/evaluate.py` | 238/238 已有字段正确，7/7 缺项保持为空；1/1 扫描件拦截 |
...
每条发票在当前浏览器页签中有独立草稿。本轮 Chrome 实测切换记录、筛选和真实刷新后，字段与说明仍保留；上传后的草稿保留已在上一轮内置浏览器验证，两者是分开的运行记录（[详情](docs/evaluation.md#browser-follow-up)）。应用有草稿时请求显示离开提醒，但原生离开提示是否出现、原生丢弃确认弹窗的交互仍未验证。关闭页签后不能保证恢复，所以应先保存。草稿不是已保存记录，也不会进入导出。

保存、确认、拒绝都要填写说明。保存后回到待确认；确认会检查本次提交的值；拒绝后不能导出。历史保留改前/改后值、说明和 UTC 时间，原始提取依据仍可查看。如果另一个页签已修改已保存记录，应用保留当前草稿并提示冲突。先核对最新保存值，再丢弃旧草稿，重新填写仍需保留的修改。也可以把每个字段还原为当前已保存值，再清空说明；这条路径已实测可清除草稿，有合格记录时导出按钮重新可用。
...
22 项 Python 检查覆盖原有流程、版本冲突和标签配置，4 项 Node 检查覆盖草稿状态。Node 只用于这些检查，运行应用不需要它。相同的应用与测试文件也在全新 Windows 11 环境、Python 3.12.14、Node 26.4.0 下通过。[运行环境及首次 CI 配置失败](docs/evaluation.md#latest-reproducible-run)另有记录。

之前的合成布局评测保留了首次独立样本 **0/66**、另一批首次测试 **35/41** 的已有字段正确数。本轮本地及 CI 重跑复现了修复后的结果：35 份文本 PDF 的 238 个已有字段和 7 个缺项全部匹配，22 份正常单据通过、13 份异常单据拦截；另有一份纯图像扫描件被拒绝。这是已知合成布局的回归表现，不是未见样本或真实业务准确率。[完整分母与失败记录](docs/evaluation.md)均保留。
